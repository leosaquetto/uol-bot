...
# - tenta localizar a mensagem espelhada no grupo de discussões
# - envia comentário na thread correta usando message_thread_id
# - envia logo do parceiro por upload de arquivo (não por url direta)
...
def find_group_mirror_message_id(
    ...
    """
    tenta descobrir qual mensagem apareceu no grupo vinculado correspondente ao post do canal.

    ordem:
    1. match por forward_origin / forward_from_message_id
    2. fallback por caption parecida
    """
        ...
        try:
            ...
            for candidate in recent_group_candidates:
                ...
                if cap == expected_caption_cmp:
                    log(f"   ✅ id espelhado encontrado no grupo por caption exata: {candidate['message_id']}")
                    return candidate["message_id"]

                if (
                    expected_caption_cmp[:120] and expected_caption_cmp[:120] in cap
                ) or (
                    cap[:120] and cap[:120] in expected_caption_cmp
                ):
                    log(f"   ✅ id espelhado encontrado no grupo por caption aproximada: {candidate['message_id']}")
                    return candidate["message_id"]
...
def send_description_comment(
    ...
    """
    envia comentário na thread correta do grupo vinculado ao canal.
    """
