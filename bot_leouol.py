...
# - copy/caption/comentário refinados sem perder a blindagem anti-duplicidade
...
def extract_hashtags(title: str, description: str, link: str) -> List[str]:
    base = f"{title}\n{description}\n{link}".lower()
    tags = []

    if "teatro" in base:
        tags.append("#teatro")
    if "ingresso" in base or "ingressos" in base:
        tags.append("#ingressos")
    if "joão caetano" in base or "joao caetano" in base:
        tags.append("#teatrojoaocaetano")
    if "sky hall" in base:
        tags.append("#skyhall")
    if "rio de janeiro" in base or "rio" in base:
        tags.append("#rj")
    if "são paulo" in base or "sao paulo" in base or "sp" in title.lower():
        tags.append("#sp")
...
def build_caption(title: str, validity: Optional[str], link: str, description: str = "") -> str:
    hashtags = extract_hashtags(title, description, link)
    ...
    parts.append("💬 detalhes completos nos comentários")

    if hashtags:
        parts.append(" ".join(escape_html(tag) for tag in hashtags))
...
def build_comment_text(title: str, description: str, validity: Optional[str], link: str) -> str:
    ...
    hashtags = extract_hashtags(title, desc, link)
    ...
    if validity:
        parts.extend([
            "",
            f"📅 <b>validade:</b> {escape_html(clean_multiline_text(validity))}"
        ])

    parts.extend([
        "",
        f"🔗 <a href=\"{escape_html(link)}\">abrir oferta no clube uol</a>",
    ])

    if hashtags:
        parts.extend([
            "",
            " ".join(escape_html(tag) for tag in hashtags)
        ])
...
def send_text_comment(title: str, description: str, validity: Optional[str], link: str, reply_to_id: int) -> bool:
    ...
    try:
        log(f"   💬 enviando comentário como reply ao ID {reply_to_id}")
...
def run_consumer() -> None:
    ...
    for index, offer in enumerate(offers, start=1):
        ...
        caption = build_caption(title, validity, link, description)
