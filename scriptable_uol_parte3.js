...
const LOCK_FILE = "uol_scriptable_parte3.lock.json"
  ...
  if (fm.fileExists(path)) {
    try {
      ...
      if (ageMs >= 0 && ageMs < LOCK_TIMEOUT_MS) {
        ...
        return { ok: false, message: `lock ativo da parte3 (iniciado em ${when}, há ${Math.trunc(ageMs / 1000)}s)` }
      }
    } catch (e) {}
  }
...
async function main() {
  ...
  if (!lock.ok) return `abortado_parte3 | ${lock.message}`
